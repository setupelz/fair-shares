...
# %% [markdown]
# ## Step 5: Preprocess international bunker fuel CO2 timeseries
#
# Extract the "Bunkers" column from the GCB2024 fossil emissions Excel file and
# save as a standard intermediate CSV.  The raw file uses MtC/yr; we convert to
# MtCO2/yr (x 3.664) for consistency with the rest of the pipeline.
#
# **Source:** Global Carbon Budget 2024 — National Fossil Carbon Emissions
# (Territorial Emissions sheet, header row 12, column "Bunkers").

# %%
from fair_shares.library.utils.data.nghgi import _MTC_TO_MTCO2
...
if "Bunkers" not in gcb_sheet.columns:
    raise DataProcessingError(
        f"'Bunkers' column not found in GCB sheet. "
        f"Available columns (last 10): {list(gcb_sheet.columns[-10:])}"
    )
...
bunker_col = bunker_col[bunker_col.index.map(_is_year_bunker)]
...
year_indices = sorted([y for y in bunker_col.index if _is_year_bunker(y)])
bunker_output = pd.DataFrame(
    [bunker_col[year_indices].values],
    columns=[str(int(y)) for y in year_indices],
    index=pd.Index(["bunkers"], name="source"),
)

bunker_output_path = intermediate_dir / "bunker_timeseries.csv"
bunker_output.reset_index().to_csv(bunker_output_path, index=False)
print(f"Saved bunker timeseries to: {bunker_output_path}")
print(f"  Year range: {min(year_indices)}-{max(year_indices)}")
print(
    f"  Last year value: {bunker_output[str(max(year_indices))].iloc[0]:.1f} MtCO2/yr"
)
