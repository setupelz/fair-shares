...
#
# Pre-configured for Remaining Carbon Budgets (RCBs) with CO2-FFI emissions.
#
# **Datasets**: PRIMAP emissions (2025), WDI GDP (2025), UN population (2025), UNU-WIDER Gini (2025)
...
# For pathway allocations only (not used with target="rcbs")
...
# %% [markdown]
# ---
# ## Step 2: Budget Allocation Approaches
#
# **Three principle-based approaches:**
#
# 1. **Equal per capita** - Operationalizes equal rights to atmosphere
# 2. **Adjusted** - Operationalizes historical responsibility and capability (CBDR-RC)
# 3. **Gini-adjusted** - Adds within-country inequality adjustment for subsistence protection
#
# Each operationalizes different ethical foundations for effort-sharing.
#
...
if absolute_parquet_path.exists():
    ...
    if not country_data.empty:
        ...
        # Debug output
        print(
            f"\nDebug: Approaches in data for {TEST_COUNTRY}: "
            f"{list(available_approaches)}"
        )
        ...
        if len(approaches) != len(configured_approaches):
            ...
            print(f"Warning: Some configured approaches not found: {missing}")
            print(
                f"Available approaches in full dataset: "
                f"{list(absolute_df['approach'].unique())}"
            )
