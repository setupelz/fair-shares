...
Running the API needs the processed input data (``setup_data``); the test skips
when neither the fixtures nor that data are present (the fixture script above
produces both).
"""
...
_ID_KEYS = [
    "iso3c", "category", "emission-category", "approach", "source",
    "climate-assessment", "quantile", "allocation-year", "first-allocation-year",
    "preserve-allocation-year-shares", "preserve-first-allocation-year-shares",
    "cumulative-end-year", "pre-allocation-responsibility-weight",
    "capability-weight", "pre-allocation-responsibility-year",
    "pre-allocation-responsibility-per-capita", "capability-per-capita",
    "pre-allocation-responsibility-exponent", "capability-exponent",
    "pre-allocation-responsibility-functional-form", "capability-functional-form",
    "capability-reference-year", "shape", "deviation-end-year",
    "nonco2-debt-mode", "convergence-year",
]
...
CASES = [
    pytest.param(
        ...
        "half-sine", 2050, None, id="co2-ffi-EPC",
    ),
    pytest.param(
        "co2-ffi",
        {"equal-per-capita-budget": {"allocation_year": 2015,
        ...
        "exponential", 2100, None, id="co2-ffi-ECPC",
    ),
    pytest.param(
        "all-ghg",
        {"equal-per-capita-budget": {"allocation_year": 2015,
                                     "preserve_allocation_year_shares": True}},
        "half-sine", 2050, "free-rider", id="all-ghg-EPC-free-rider",
    ),
    pytest.param(
        "all-ghg",
        {"equal-per-capita-budget": {"allocation_year": 2015,
                                     "preserve_allocation_year_shares": True}},
        "exponential", 2075, "co2-debit", id="all-ghg-EPC-co2-debit",
    ),
    pytest.param(
        "all-ghg",
        {"per-capita-adjusted-budget": {"allocation_year": 2015,
                                        ...
                                        "preserve_allocation_year_shares": True}},
        "half-sine", 2050, "co2-debit", id="all-ghg-PCA-capability",
    ),
    pytest.param(
        ...
        {"equal-per-capita-budget": {"allocation_year": 2015,
                                     "cumulative_end_year": 2050,
                                     "preserve_allocation_year_shares": False}},
        "half-sine", 2100, "free-rider", id="all-ghg-ex-ECPC",
    ),
]
