...
**Capability snapshot mode (`capability_reference_year: int | None = None`):** By default, capability is computed year-by-year — `pf(GDP_per_cap(g, t))` is evaluated for each country and year in the allocation window, then combined with that year's population. Setting `capability_reference_year` to an integer freezes capability at a single reference year instead: `pf(GDP_per_cap(g, t_ref))` is computed once per country from that year's data and broadcast across the rest of the window. Available on the adjusted approaches — `per_capita_adjusted_budget`, `per_capita_adjusted_gini_budget`, `per_capita_adjusted`, and `per_capita_adjusted_gini` — with identical semantics on the budget and pathway side; not available on `equal_per_capita*` (no capability term to freeze) or the convergence family. The reference year can be before, at, or after `allocation_year`. Year-by-year and snapshot modes are algebraically equivalent only when capability is constant over the window; in practice they diverge substantially for long windows spanning periods of economic change. See [capability_reference_year](parameter-effects.md#capability_reference_year) for the source-case mechanics (in-window, beyond-last-observed-year, pre-window) and the Gini interaction.
...
Year-by-year integration is the more defensible forward-looking semantic — it reflects lifetime capability over the allocation window rather than a point-in-time snapshot — but snapshot is the reproduction-faithful choice for papers that defined capability as a fixed indicator at a reference instant.
