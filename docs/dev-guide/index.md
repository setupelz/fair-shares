...
Both return `relative_shares_cumulative_emission` (summing to 1) and methods to compute absolute emissions.
...
uv run pytest tests/test_allocations.py
...
- Documentation examples with `✅` or `❌` markers (intentionally incomplete)
